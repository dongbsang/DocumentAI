...
class OCRService:
    ...
    def extract_text_tesseract(self, image, preprocess=True):
        ...
        try:
            ...
            # OCR 신뢰도 측정
            try:
                data = pytesseract.image_to_data(image, lang='kor+eng', config=custom_config, output_type=pytesseract.Output.DICT)
    ...
    def _preprocess_image_advanced(self, image):
        ...
        try:
            ...
            if stat[1] - stat[0] < 100:  # 대비가 낮은 경우
                ...
                logger.info(f"  ✨ 대비 향상 완료 (강함)")
            else:
                image = enhancer.enhance(1.3)
                logger.info(f"  ✨ 대비 향상 완료 (보통)")
            ...
            if CV2_AVAILABLE:
                ...
                if noise_level < 100:  # 노이즈가 많은 경우
                    ...
                    logger.info(f"  🧹 노이즈 제거 완료 (강함)")
                else:
                    denoised = cv2.GaussianBlur(gray, (3, 3), 0)
                    logger.info(f"  🧹 노이즈 제거 완료 (가벼움)")
    ...
    def _preprocess_image_for_easyocr(self, image):
        ...
        try:
            ...
            enhancer = ImageEnhance.Contrast(image)
            image = enhancer.enhance(1.3)
            logger.info(f"  ✨ 대비 향상 완료")

            # 3. 선명도 향상
            enhancer = ImageEnhance.Sharpness(image)
            image = enhancer.enhance(1.5)
            logger.info(f"  🔍 선명도 향상 완료")
